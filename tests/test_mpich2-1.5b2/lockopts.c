...
{ 
    ...
    foMPI_Win_create( rmabuf, memsize*sizeof(int), sizeof(int), MPI_INFO_NULL, 
		    MPI_COMM_WORLD, &win );
	...
	foMPI_Win_lock( foMPI_LOCK_SHARED, 0, 0, win );
	foMPI_Accumulate( srcbuf, 1, vectype, 0, 0, 10, MPI_INT, foMPI_SUM, win );
	foMPI_Win_unlock( 0, win );
	MPI_Barrier( MPI_COMM_WORLD );
    }
    else {
  ...
  MPI_Barrier( MPI_COMM_WORLD );
	...
	foMPI_Win_lock( foMPI_LOCK_SHARED, 0, 0, win );
	foMPI_Put( srcbuf, 1, vectype, 0, 0, 10, MPI_INT, win );
	foMPI_Win_unlock( 0, win );
	MPI_Barrier( MPI_COMM_WORLD );
    }
    else {
	...
	int val;
	foMPI_Win_lock( foMPI_LOCK_SHARED, 0, 0, win );
	foMPI_Get( &val, 1, MPI_INT, 0, 10, 1, MPI_INT, win );
	foMPI_Put( srcbuf, 1, vectype, 0, 0, 10, MPI_INT, win );
	foMPI_Win_unlock( 0, win );
	MPI_Barrier( MPI_COMM_WORLD );
	if (val != -10) {
