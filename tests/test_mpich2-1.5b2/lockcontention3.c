...
void RMATest( int i, foMPI_Win win, int master, int *srcbuf, int srcbufsize, int *getbuf, int getbufsize );
int  RMACheck( int i, int *buf, MPI_Aint bufsize );
int  RMACheckGet( int i, foMPI_Win win, int *getbuf, MPI_Aint getsize);
...
int main( int argc, char *argv[] )
{
    ...
    foMPI_Win_create( rmabuffer, bufsize*sizeof(int), sizeof(int), MPI_INFO_NULL,
		    MPI_COMM_WORLD, &win );
	...
	if (wrank == master) {
	    ...
	    /* Because this lock is local, it must return only when the
	     lock is acquired */
	    foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, 0, master, win );
...
void RMATest( int i, foMPI_Win win, int master, int *srcbuf, int srcbufsize, int *getbuf, int getbufsize )
{
	...
	source[0] = PUT_VAL;
	foMPI_Put( source, 1, MPI_INT, master, OFFSET_1, 1, MPI_INT, win );
	break;
    case 1: /* Single short accumulate (1 word of value 17 at OFFSET_2) */
	source[0] = ACC_VAL;
	foMPI_Accumulate( source, 1, MPI_INT, master, 
			OFFSET_2, 1, MPI_INT, foMPI_SUM, win );
	break;
    case 2: /* Single short get (1 word at OFFSET_3) */
	getbuf[0] = -1;
	foMPI_Get( getbuf, 1, MPI_INT, master, OFFSET_3, 1, MPI_INT, win );
	break;
    case 3: /* Datatype single put (strided put) */
	for (j=0; j<veccount; j++) {
	    source[j*stride] = PUT_VAL + j;
	}
	foMPI_Put( source, 1, vectype, master, OFFSET_1, 1, vectype, win );
	break;
    case 4: /* Datatype single accumulate (strided acc) */
	for (j=0; j<veccount; j++) {
	    source[j*stride] = ACC_VAL + j;
	}
	foMPI_Accumulate( source, 1, vectype, master, 
			OFFSET_2, 1, vectype, foMPI_SUM, win );
	break;
    case 5: /* Datatype single get (strided get) */
	for (j=0; j<veccount; j++) {
	    getbuf[j] = -j;
	}
	foMPI_Get( getbuf, 1, vectype, master,
	...
	for (j=0; j<veccount; j++) {
	    foMPI_Put( source + j*stride, 1, MPI_INT, master, 
	...
	for (j=0; j<veccount; j++) {
	    foMPI_Accumulate( source + j*stride, 1, MPI_INT, master, 
			    OFFSET_2+j*stride, 1, MPI_INT, foMPI_SUM, win );
	}
	break;
	...
	for (j=0; j<mednum; j++) {
	    ...
	    foMPI_Put( source + j*medcount, medcount, MPI_INT, master,
	...
	for (j=0; j<mednum; j++) {
	    ...
	    foMPI_Accumulate( source + j*medcount, medcount, MPI_INT, master,
	...
	for (j=0; j<mednum; j++) {
	    ...
	    foMPI_Get( getbuf + j*medcount, medcount, MPI_INT, master,
...
int RMACheck( int i, int *buf, MPI_Aint bufsize )
{
    ...
    case 6: /* a few small puts (like strided put, but 1 word at a time) */
	for (j=0; j<veccount;j++) {
	    if (buf[OFFSET_1+j*stride] != PUT_VAL + j) {
		errs++;
		printf( "case %d: value is %d should be %d\n", i,
			buf[OFFSET_1+j*stride], PUT_VAL+j );
	    }
	}
	break;
    case 4: /* Datatype single accumulate (strided acc) */
    case 7: /* a few small accumulates (like strided acc, but 1 word at a time )*/
	for (j=0; j<veccount ; j++) {
	    if (buf[OFFSET_2+j*stride] != ACC_VAL + j + OFFSET_2 + j*stride) {
		errs++;
		printf( "case %d: value is %d should be %d\n", i,
			buf[OFFSET_2+j*stride], ACC_VAL+j+OFFSET_2+j*stride );
	    }
	}
	break;
    case 5: /* Datatype single get (strided get) */
