...
int main(int argc, char *argv[])
{
    ...
    if (rank < 3) {
        ...
        foMPI_Win_create(B, SIZE2*sizeof(int), sizeof(int), MPI_INFO_NULL, CommThree, &win);

        if (rank == 0) {
            for (i=0; i<SIZE1; i++) {
                foMPI_Win_lock(foMPI_LOCK_EXCLUSIVE, trank, 0, win);
                foMPI_Put(A+i, 1, MPI_INT, trank, i, 1, MPI_INT, win);
            ...
            for (i=0; i<SIZE1; i++) {
                foMPI_Win_lock(foMPI_LOCK_EXCLUSIVE, trank, 0, win);
                foMPI_Get(A+i, 1, MPI_INT, trank, SIZE1+i, 1, MPI_INT, win);
                foMPI_Win_unlock(trank, win);
            }
