...
{ 
    ...
    /* Create the window */
    foMPI_Win_create(rmabuf, windowsize*sizeof(int), sizeof(int), MPI_INFO_NULL, 
		   MPI_COMM_WORLD, &win); 

    /* Multiple puts, with contention at trank */
    MPI_Barrier( MPI_COMM_WORLD );
    for (i=0; i<NELM; i++) {
	foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, trank, 0, win );
	foMPI_Put( &localbuf[i], 1, MPI_INT, trank, 
		 rank + i*wsize, 1, MPI_INT, win );
	foMPI_Put( &localbuf[i], 1, MPI_INT, trank, 
		 rank + (i+NELM)*wsize, 1, MPI_INT, win );
	foMPI_Win_unlock( trank, win );
    }
    MPI_Barrier( MPI_COMM_WORLD );
    if (rank == trank) {
	foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, trank, 0, win );
    ...
    for (i=0; i<NELM; i++) {
	foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, trank, 0, win );
	foMPI_Put( &localbuf[i], 1, MPI_INT, trank, rank + i*wsize, 1, MPI_INT, 
		 win );
	foMPI_Win_unlock( trank, win );
    }
    MPI_Barrier( MPI_COMM_WORLD );
	...
	foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, trank, 0, win );
	if (rank != trank) {
	    foMPI_Put( &localbuf2[i*NBLOCK], NBLOCK, MPI_INT, trank, 
	...
	foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, trank, 0, win );
	if (rank != trank) {
	    foMPI_Put( &localbuf2[i], NBLOCK, MPI_INT, trank, 
	    ...
	    foMPI_Get( &vals[i], 1, MPI_INT, trank, i, 1, MPI_INT, win );
	}
	foMPI_Win_unlock( trank, win );
    }
    MPI_Barrier( MPI_COMM_WORLD );
    ...
    for (i=0; i<NELM; i++) {
	foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, trank, 0, win );
	foMPI_Accumulate( &localbuf[i], 1, MPI_INT, trank, rank+i*wsize, 
			1, MPI_INT, foMPI_SUM, win );
	foMPI_Accumulate( &localbuf[i], 1, MPI_INT, trank, rank+i*wsize, 
			1, MPI_INT, foMPI_SUM, win );
	foMPI_Win_unlock( trank, win );
    }
