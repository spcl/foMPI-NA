...
int main(int argc, char **argv) {
    ...
    */
    ...
    foMPI_Win_create(win_buf, bufsize, 1, MPI_INFO_NULL, MPI_COMM_WORLD, &buf_win);
...
#ifdef ABSOLUTE
      foMPI_Accumulate(MPI_BOTTOM, 1, src_type, peer, 0, 1, dst_type, foMPI_SUM, buf_win);
#else
      foMPI_Accumulate(src_buf, 1, src_type, peer, 0, 1, dst_type, foMPI_SUM, buf_win);
#endif

      foMPI_Win_unlock(peer, buf_win);

      foMPI_Type_free(&src_type);
      foMPI_Type_free(&dst_type);
    }
