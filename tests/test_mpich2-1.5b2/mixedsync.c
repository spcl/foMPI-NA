...
int main( int argc, char *argv[] )
{
	...
	if (crank == source) {
	    ...
	    foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, dest, 0, win );
	    foMPI_Put( buf0, count0, MPI_INT, dest, 0, count0, MPI_INT, win );
	    foMPI_Put( buf1, count1, MPI_INT, dest, count0, count1, MPI_INT, 
		     win );
	    foMPI_Put( buf2, count2, MPI_INT, dest, count0+count1, count2, 
		     MPI_INT, win );
	    foMPI_Win_unlock( dest, win );
	...
	if (crank == source) {
	    foMPI_Put( buf0, count0, MPI_INT, dest, 1, count0, MPI_INT, win );
	    foMPI_Put( buf1, count1, MPI_INT, dest, 1+count0, count1, MPI_INT, 
		     win );
	    foMPI_Put( buf2, count2, MPI_INT, dest, 1+count0+count1, count2, 
		     MPI_INT, win );
	}
	foMPI_Win_fence( 0, win );
	...
	if (crank == source) {
	    foMPI_Win_lock( foMPI_LOCK_EXCLUSIVE, dest, 0, win );
	    foMPI_Accumulate( buf0, count0, MPI_INT, dest, 0, count0, MPI_INT, 
			    foMPI_SUM, win );
	    foMPI_Accumulate( buf1, count1, MPI_INT, dest, count0, count1, 
			    MPI_INT, foMPI_SUM, win );
	    foMPI_Put( buf2, count2, MPI_INT, dest, count0+count1, count2, 
		     MPI_INT, win );
	    foMPI_Win_unlock( dest, win );
	}
	...
	foMPI_Win_fence( 0, win );
	if (crank == source) {
	    foMPI_Accumulate( buf0, count0, MPI_INT, dest, 1, count0, MPI_INT, 
			    foMPI_REPLACE, win );
	    foMPI_Accumulate( buf1, count1, MPI_INT, dest, 1+count0, count1, 
			    MPI_INT, foMPI_REPLACE, win );
	    foMPI_Put( buf2, count2, MPI_INT, dest, 1+count0+count1, count2, 
		     MPI_INT, win );
	}
	foMPI_Win_fence( 0, win );
	...
	if (crank == source) {
	    ...
	    foMPI_Put( buf1, count1, MPI_INT, dest, count0, count1, MPI_INT, 
		     win );
	    foMPI_Get( inbuf2, count2, MPI_INT, dest, count0+count1, count2, 
		     MPI_INT, win );
	    foMPI_Win_unlock( dest, win );
	}
	else if (crank == dest) {
	...
	if (crank == source) {
	    ...
	    foMPI_Put( buf1, count1, MPI_INT, dest, 1+count0, count1, MPI_INT, 
		     win );
	    foMPI_Get( inbuf2, count2, MPI_INT, dest, 1+count0+count1, count2, 
		     MPI_INT, win );
	}
	foMPI_Win_fence( 0, win );
