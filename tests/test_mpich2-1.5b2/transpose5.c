...
{ 
    ...
    if (rank < 2)
    {
        ...
        if (rank == 0)
        {
            ...
            foMPI_Accumulate(&A[0][0], NROWS*NCOLS, MPI_INT, 1, 0, 1, xpose, foMPI_SUM, win);
        ...
        { /* rank=1 */
            for (i=0; i<NROWS; i++)
                for (j=0; j<NCOLS; j++)
                    A[i][j] = i*NCOLS + j;
            foMPI_Win_create(&A[0][0], NROWS*NCOLS*sizeof(int), sizeof(int), MPI_INFO_NULL, CommDeuce, &win);
            foMPI_Win_fence(0, win);
