...
void Get_nextval_tree(foMPI_Win win, int *get_array, MPI_Datatype get_type,
...
{ 
    ...
    if (rank == 0) {
        ...
        foMPI_Win_create(counter_mem, pof2*2*sizeof(int), sizeof(int),
...
*/
        ...
        free(get_idx);
        free(acc_idx);
        foMPI_Type_free(&get_type);
        foMPI_Type_free(&acc_type);
                      ...
                      MPI_Datatype acc_type, int nlevels, int *value)
{
    ...
    foMPI_Accumulate(one, nlevels, MPI_INT, 0, 0, 1, acc_type,
                   foMPI_SUM, win);
    foMPI_Get(get_array, nlevels, MPI_INT, 0, 0, 1, get_type, win);
