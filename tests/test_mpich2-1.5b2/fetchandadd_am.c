...
void Get_nextval(foMPI_Win win, int *val_array, MPI_Datatype get_type,
...
{ 
    ...
    if (rank == 0) {
        ...
        foMPI_Win_create(counter_mem, nprocs*sizeof(int), sizeof(int),
                 ...
                 int rank, int nprocs, int *value) 
{
    ...
    foMPI_Accumulate(&one, 1, MPI_INT, 0, rank, 1, MPI_INT, foMPI_SUM, win);
    foMPI_Get(val_array, 1, get_type, 0, 0, 1, get_type, win); 
