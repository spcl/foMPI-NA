...
{ 
    ...
    if (rank < 2) {
        ...
        if (rank == 0) {
            ...
            for (i=0; i<SIZE; i++) {
                A[i] = i+100;
                foMPI_Get(&A[i], 1, MPI_INT, 1, i, 1, MPI_INT, win);
            }
            foMPI_Win_fence(0, win);
            for (i=0; i<SIZE; i++)
                if (A[i] != 1000 + i) {
        ...
        if (rank == 1) {
            for (i=0; i<SIZE; i++)
                A[i] = 1000 + i;
            foMPI_Win_create(A, SIZE*sizeof(int), sizeof(int), MPI_INFO_NULL, CommDeuce, &win);
            foMPI_Win_fence(0, win);
            for (i=0; i<SIZE; i++) {
                B[i] = i+200;
                foMPI_Get(&B[i], 1, MPI_INT, 0, i, 1, MPI_INT, win);
            }
            foMPI_Win_fence(0, win);
            for (i=0; i<SIZE; i++)
                if (B[i] != 500 + i) {
