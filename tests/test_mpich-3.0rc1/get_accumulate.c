...
void reset_bufs(TYPE_C *win_ptr, TYPE_C *res_ptr, TYPE_C *val_ptr, TYPE_C value, foMPI_Win win) {
    int rank, nproc, i;
...
int main(int argc, char **argv) {
    ...
    foMPI_Win_create(win_ptr, sizeof(TYPE_C)*nproc*COUNT, sizeof(TYPE_C),
    ...
    for (i = 0; i < ITER; i++) {
        foMPI_Win_lock(foMPI_LOCK_EXCLUSIVE, rank, 0, win);
        foMPI_Get_accumulate(val_ptr, COUNT, TYPE_MPI, res_ptr, COUNT, TYPE_MPI, 
                            rank, 0, COUNT, TYPE_MPI, foMPI_SUM, win);
        foMPI_Win_unlock(rank, win);
    }
    ...
    for (i = 0; i < ITER; i++) {
        ...
        foMPI_Get_accumulate(val_ptr, COUNT, TYPE_MPI, res_ptr, COUNT, TYPE_MPI, 
                            (rank+1)%nproc, 0, COUNT, TYPE_MPI, foMPI_SUM, win);
    ...
    if (rank != 0) {
        for (i = 0; i < ITER; i++) {
            ...
            foMPI_Get_accumulate(val_ptr, COUNT, TYPE_MPI, res_ptr, COUNT, TYPE_MPI, 
                                0, 0, COUNT, TYPE_MPI, foMPI_SUM, win);
    ...
    for (i = 0; i < ITER; i++) {
        ...
        foMPI_Win_fence(MPI_MODE_NOPRECEDE, win);
        for (j = 0; j < nproc; j++) {
            foMPI_Get_accumulate(val_ptr, COUNT, TYPE_MPI, &res_ptr[j*COUNT], COUNT, TYPE_MPI,
                                j, rank*COUNT, COUNT, TYPE_MPI, foMPI_SUM, win);
        }
        foMPI_Win_fence(MPI_MODE_NOSUCCEED, win);
        MPI_Barrier(MPI_COMM_WORLD);
    ...
    for (i = 0; i < ITER; i++) {
        ...
        foMPI_Win_lock(foMPI_LOCK_EXCLUSIVE, target, 0, win);
        foMPI_Get_accumulate(NULL, COUNT, TYPE_MPI, res_ptr, COUNT, TYPE_MPI,
                            target, 0, COUNT, TYPE_MPI, foMPI_NO_OP, win);
        foMPI_Win_unlock(target, win);
        ...
        foMPI_Win_lock(foMPI_LOCK_EXCLUSIVE, target, 0, win);
        foMPI_Get_accumulate(NULL, 0, TYPE_MPI, res_ptr, COUNT, TYPE_MPI,
                            target, 0, COUNT, TYPE_MPI, foMPI_NO_OP, win);
        foMPI_Win_unlock(target, win);
        ...
        foMPI_Win_lock(foMPI_LOCK_EXCLUSIVE, target, 0, win);
        foMPI_Get_accumulate(NULL, 0, MPI_DATATYPE_NULL, res_ptr, COUNT, TYPE_MPI,
                            target, 0, COUNT, TYPE_MPI, foMPI_NO_OP, win);
        foMPI_Win_unlock(target, win);
