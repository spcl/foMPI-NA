...
int main(int argc, char **argv) {
    ...
    foMPI_Win_lock_all(0, llist_win);
    ...
    for (i = 0; i < NUM_ELEMS; i++) {
        ...
        do {
            ...
            foMPI_Win_flush(tail_ptr.rank, llist_win);
            success = (next_tail_ptr.rank == nil.rank);

            if (success) {
                int i, flag;

                foMPI_Accumulate(&new_elem_ptr.disp, 1, MPI_AINT, tail_ptr.rank,
                               (MPI_Aint) &(((llist_elem_t*)tail_ptr.disp)->next.disp), 1,
                               MPI_AINT, foMPI_REPLACE, llist_win);
                ...
                do {
                    foMPI_Get_accumulate( NULL, 0, MPI_DATATYPE_NULL, &next_tail_ptr.disp,
                                        1, MPI_AINT, tail_ptr.rank,
                                        (MPI_Aint) &(((llist_elem_t*)tail_ptr.disp)->next.disp),
                                        1, MPI_AINT, foMPI_NO_OP, llist_win);
                    foMPI_Win_flush(tail_ptr.rank, llist_win);
    ...
    foMPI_Win_unlock_all(llist_win);
