...
void RunAccFence( foMPI_Win win, int destRank, int cnt, int sz );
void RunAccLock( foMPI_Win win, int destRank, int cnt, int sz );
void RunPutFence( foMPI_Win win, int destRank, int cnt, int sz );
void RunPutLock( foMPI_Win win, int destRank, int cnt, int sz );
void RunAccPSCW( foMPI_Win win, int destRank, int cnt, int sz, 
		 MPI_Group exposureGroup, MPI_Group accessGroup );
void RunPutPSCW( foMPI_Win win, int destRank, int cnt, int sz, 
		 MPI_Group exposureGroup, MPI_Group accessGroup );
...
int main( int argc, char *argv[] )
{
    ...
    foMPI_Win_create( arraybuffer, arraysize*sizeof(int), (int)sizeof(int),
...
void RunAccFence( foMPI_Win win, int destRank, int cnt, int sz )
{
	...
	j = 0;
	for (i=0; i<cnt; i++) {
	    foMPI_Accumulate( &one, sz, MPI_INT, destRank, 
			    j, sz, MPI_INT, foMPI_SUM, win );
	    j += sz;
	}
	foMPI_Win_fence( 0, win );
    }
...
void RunAccLock( foMPI_Win win, int destRank, int cnt, int sz )
{
    ...
    for (k=0; k<MAX_RUNS; k++) {
	MPI_Barrier( MPI_COMM_WORLD );
	foMPI_Win_lock( foMPI_LOCK_SHARED, destRank, 0, win );
	j = 0;
	for (i=0; i<cnt; i++) {
	    foMPI_Accumulate( &one, sz, MPI_INT, destRank, 
			    j, sz, MPI_INT, foMPI_SUM, win );
...
void RunPutFence( foMPI_Win win, int destRank, int cnt, int sz )
{
	...
	j = 0;
	for (i=0; i<cnt; i++) {
	    foMPI_Put( &one, sz, MPI_INT, destRank, 
			    j, sz, MPI_INT, win );
	    j += sz;
	}
	foMPI_Win_fence( 0, win );
...
void RunPutLock( foMPI_Win win, int destRank, int cnt, int sz )
{
	...
	j = 0;
	for (i=0; i<cnt; i++) {
	    foMPI_Put( &one, sz, MPI_INT, destRank, j, sz, MPI_INT, win );
	    j += sz;
	}
	foMPI_Win_unlock( destRank, win );
    }
		 ...
		 MPI_Group exposureGroup, MPI_Group accessGroup )
{
    ...
    for (k=0; k<MAX_RUNS; k++) {
	MPI_Barrier( MPI_COMM_WORLD );
	foMPI_Win_post( exposureGroup, 0, win );
	foMPI_Win_start( accessGroup, 0, win );
	j = 0;
	for (i=0; i<cnt; i++) {
	    foMPI_Put( &one, sz, MPI_INT, destRank, j, sz, MPI_INT, win );
	    j += sz;
