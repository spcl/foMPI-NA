...
int main( int argc, char *argv[] )
{
    ...
    foMPI_Win_create(buf, sizeof(int)*(rank == 0), 1, MPI_INFO_NULL, MPI_COMM_WORLD, &window);

    foMPI_Win_lock(foMPI_LOCK_SHARED, 0, 0, window);

    /* GET-ACC: Test third-party communication, through rank 0. */
    for (i = 0; i < ITER; i++) {
        foMPI_Request gacc_req;
        int val = -1, exp = -1;
        ...
        foMPI_Rget_accumulate(&rank, 1, MPI_INT, &val, 1, MPI_INT, 0, 0, 1, MPI_INT, foMPI_REPLACE, window, &gacc_req);
        foMPI_Wait(&gacc_req, MPI_STATUS_IGNORE);
    ...
    for (i = 0; i < ITER; i++) {
        ...
        foMPI_Rget(&val, 1, MPI_INT, 0, 0, 1, MPI_INT, window, &req);
        foMPI_Wait(&req, MPI_STATUS_IGNORE);

        foMPI_Rput(&rank, 1, MPI_INT, 0, 0, 1, MPI_INT, window, &req);
        foMPI_Wait(&req, MPI_STATUS_IGNORE);
    ...
    for (i = 0; i < ITER; i++) {
        ...
        foMPI_Raccumulate(&rank, 1, MPI_INT, 0, 0, 1, MPI_INT, foMPI_REPLACE, window, &req);
        foMPI_Wait(&req, MPI_STATUS_IGNORE);
