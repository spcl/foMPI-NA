...
MPI_Aint alloc_elem(int value, foMPI_Win win) {
    MPI_Aint disp;
    ...
    elem_ptr->next  = nil;
    foMPI_Win_attach(win, elem_ptr, sizeof(llist_elem_t));
...
int main(int argc, char **argv) {
    ...
    foMPI_Win_create_dynamic(MPI_INFO_NULL, MPI_COMM_WORLD, &llist_win);
    ...
    for (i = 0; i < NUM_ELEMS; i++) {
        ...
        do {
            ...
            foMPI_Win_lock(foMPI_LOCK_SHARED, tail_ptr.rank, MPI_MODE_NOCHECK, llist_win);

            foMPI_Compare_and_swap((void*) &new_elem_ptr.rank, (void*) &nil.rank,
                                  (void*) &next_tail_ptr.rank, MPI_INT, tail_ptr.rank,
                                  (MPI_Aint) &(((llist_elem_t*)tail_ptr.disp)->next.rank), llist_win);

            foMPI_Win_unlock(tail_ptr.rank, llist_win);
            ...
            if (success) {
                ...
                foMPI_Fetch_and_op(&new_elem_ptr.disp, &result, MPI_AINT, tail_ptr.rank,
                                  (MPI_Aint) &(((llist_elem_t*)tail_ptr.disp)->next.disp),
                                  foMPI_REPLACE, llist_win);
                ...
                */
                ...
                do {
                    ...
                    foMPI_Fetch_and_op(NULL, &next_tail_ptr.disp, MPI_AINT, tail_ptr.rank,
                                      (MPI_Aint) &(((llist_elem_t*)tail_ptr.disp)->next.disp),
                                      foMPI_NO_OP, llist_win);
    ...
    if (procid == 0) {
        ...
        while (tail_ptr.disp != nil.disp) {
            ...
            foMPI_Get(&elem, sizeof(llist_elem_t), MPI_BYTE,
