...
int main(int argc, char **argv) {
    ...
    foMPI_Win_allocate_shared(sizeof(int)*ELEM_PER_PROC, sizeof(int), MPI_INFO_NULL, 
                             shm_comm, &my_base, &shm_win);

    /* Locate absolute base */
    foMPI_Win_shared_query(shm_win, MPI_PROC_NULL, &size, &disp_unit, &base); 
    ...
    foMPI_Win_lock_all(MPI_MODE_NOCHECK, shm_win);
    ...
    foMPI_Win_unlock_all(shm_win);
    foMPI_Win_free(&shm_win);
