      subroutine test_mpi3rma_get_acc_no_op_lock( base, array_size, DIM1, win, comm )
      ...
      write (filename,*) "get_acc_no_op_lock.f90"
      ...
      allocate( recv_buf(commsize), stat=mpierr )
      ...
      call foMPI_Get_accumulate( base(1), DIM1, MPI_INTEGER, recv_buf(1), DIM1, MPI_INTEGER, commsize-rank-1, start, DIM1, &
        MPI_INTEGER, foMPI_NO_OP, win, mpierr )
      ...
      do i=1,DIM1
        if( base(DIM1+i) .ne. (commsize+rank+i) ) then
          write (*,*) "Error on rank ", rank, " in ", trim(filename), &
            " line ", 73, ": element ", DIM1+i, " is ", base(DIM1+i), &
            " instead of ", commsize+rank+i, "."
          STOP
        endif
        if( recv_buf(i) .ne. (commsize+(commsize-rank-1)+i) ) then
          write (*,*) "Error on rank ", rank, " in ", trim(filename), &
            " line ", 73, ": element ", i, " is ", recv_buf(i), &
      ...
      end subroutine test_mpi3rma_get_acc_no_op_lock
