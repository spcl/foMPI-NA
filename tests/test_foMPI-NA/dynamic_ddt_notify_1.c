...
void test_mpi3rma_dynamic_ddt_notify_1(int* base, foMPI_Win win, MPI_Comm comm) {
	...
	status = foMPI_Notify_init(win, receive_from, tag, 1,
	...
	MPI_Sendrecv_replace(&start, 1, MPI_AINT, send_to, 0, receive_from, 0, comm,
			MPI_STATUS_IGNORE);

	status = foMPI_Win_fence(0, win);
	_check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);

	status = foMPI_Put_notify(&send_buffer[0], 8, MPI_INT, send_to, start, 1,
	...
	if (flag) {
		fprintf(stderr,
				"Error on rank %i in %s line %i: Multiple notification on single put.\n",
	...
	for (k = 0; k < 2; k++) {
		for (i = 0; i < 2; i++) {
			if (base[2 * local_DIM * k + i]
					 != 3 * local_DIM * (commsize - rank - 1) + 4 * k + i) {
				fprintf(stderr,
						"Error on rank %i in %s line %i: element %i is %i instead of %i.\n",
						rank, __FILE__, __LINE__, 2 * local_DIM * k + i,
						base[2 * local_DIM * k + i],
						3 * local_DIM * (commsize - rank - 1) + 4 * k + i);
				exit(EXIT_FAILURE);
			}
		}
		for (i = 0; i < 2; i++) {
			if (base[2 * local_DIM * k + i + 3]
					 != 3 * local_DIM * (commsize - rank - 1) + k * 4 + 2 + i) {
				fprintf(stderr,
						"Error on rank %i in %s line %i: element %i is %i instead of %i.\n",
						rank, __FILE__, __LINE__, 2 * local_DIM * k + i + 3,
						base[2 * local_DIM * k + i + 3],
						3 * local_DIM * (commsize - rank - 1) + k * 4 + 2 + i);
				exit(EXIT_FAILURE);
			}
		}
		if (base[k * 2 * local_DIM + 2] != -1) {
			fprintf(stderr,
					"Error on rank %i in %s line %i: element %i is %i instead of %i.\n",
					rank, __FILE__, __LINE__, k * 2 * local_DIM + 2,
					base[k * 2 * local_DIM + 2], -1);
			exit(EXIT_FAILURE);
		}
	}
