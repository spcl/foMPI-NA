...
void test_mpi3rma_get_acc_no_op_lock( int* base, int DIM, foMPI_Win win, MPI_Comm comm ) {
  ...
  recv_buf = malloc( commsize * sizeof(int) );
  ...
  status = foMPI_Get_accumulate( &base[0], DIM, MPI_INT, &recv_buf[0], DIM, MPI_INT, commsize-rank-1, start, DIM, MPI_INT, foMPI_NO_OP, win);
  _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
