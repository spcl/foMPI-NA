...
int main( int argc, char *argv[] ) {
  ...
  if (gcommsize < 4) {
    printf("The tests need at least four processes. So please rerun it.\n");
    foMPI_Finalize();
    return 0;
  }
    ...
    for ( j=0 ; j<3 ; j++ ) {
      ...
      switch (j) {
        case 0:
          ...
          status = foMPI_Win_create(&base[5], DIM*sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win2 );
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          status = foMPI_Win_create(base, 3*DIM*sizeof(int), sizeof(int), MPI_INFO_NULL, comm, &win);
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          snprintf(create_flavor, 20, "create");
          break;
        case 1: 
          status = foMPI_Win_allocate( 3*DIM*sizeof(int), sizeof(int), MPI_INFO_NULL, comm, &base, &win);
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          status = foMPI_Win_allocate( DIM*sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &base2, &win2);
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
...
//=============================================================================
      ...
      if ( j == 2 ) {
        foMPI_Win_detach( win2, base );
        _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
        foMPI_Win_attach( win2, base, 3*DIM*sizeof(int) );
        _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
        MPI_Barrier( MPI_COMM_WORLD );
      }
...
//=============================================================================
      ...
      if ( j == 2 ) {
        ...
        foMPI_Win_attach( win, &base[0], DIM*sizeof(int) );
        _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
        foMPI_Win_attach( win, &base[DIM], DIM*sizeof(int) );
        _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
        foMPI_Win_attach( win, &base[2*DIM], DIM*sizeof(int) );
        _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
...
//=============================================================================
      ...
      if (j == 2) {
        if (rank == 0) {
          printf("test %3i: %s - Put with DDTs (in more than one memory region) (fence)\n", test_counter++, create_flavor);
        }
...
//=============================================================================
        ...
        if (rank == 0) {
          printf("test %3i: %s - Accumulate/SUM with DDTs (in more than one memory region) (fence)\n", test_counter++, create_flavor);
        }
...
//=============================================================================
        ...
        if (rank == 0) {
          printf("test %3i: %s - Put with DDTs (in more than one memory region/contiguous blocks aren't aligned with memory regions boundaries) (fence)\n", test_counter++, create_flavor);
        }
...
//=============================================================================
        ...
        if (rank == 0) {
          printf("test %3i: %s - Put (contiguous over more than one memory regions) (fence)\n", test_counter++, create_flavor);
        }
...
//=============================================================================
        ...
        if (rank == 0) {
          printf("test %3i: %s - Accumulate/SUM with DDTs (in more than one memory region/contiguous blocks aren't aligned with memory regions boundaries) (fence)\n", test_counter++, create_flavor);
        }
...
//=============================================================================
        ...
        if (rank == 0) {
          printf("test %3i: %s - Accumulate/SUM (contiguous over more than one memory regions) (fence)\n", test_counter++, create_flavor);
        }
...
//=============================================================================
      ...
      switch (j) {
        case 0:
          ...
          status = foMPI_Win_create(&base3[0], DIM * sizeof(float_int_t), sizeof(float_int_t), MPI_INFO_NULL, comm, &win );
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          break;
        case 1:
          status = foMPI_Win_allocate( DIM * sizeof(float_int_t), sizeof(float_int_t), MPI_INFO_NULL, comm, &base3, &win);
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          break;
        case 2:
          base3 = malloc( DIM * sizeof(float_int_t) );
...
//=============================================================================
      ...
      switch (j) {
        case 0:
          ...
          status = foMPI_Win_create(&base4[0], sizeof(int64_t), sizeof(int64_t), MPI_INFO_NULL, comm, &win );
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          break;
        case 1:
          status = foMPI_Win_allocate( sizeof(int64_t), sizeof(int64_t), MPI_INFO_NULL, comm, &base4, &win);
          _check_fompi_status(status, MPI_SUCCESS, (char*) __FILE__, __LINE__);
          break;
        case 2:
          base4 = malloc( sizeof(int64_t) );
