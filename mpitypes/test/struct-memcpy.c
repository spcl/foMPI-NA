...
int single_struct_test(void)
{
    ...
    MPI_Type_size(mystruct, &bufsize);
    buffer = (char *) malloc(bufsize);

    MPIT_Type_init(mystruct);
    err = MPIT_Type_memcpy(&ts1, 1, mystruct, buffer, MPIT_MEMCPY_FROM_USERBUF,
			   0, bufsize);
    if (err != MPI_SUCCESS) {
	errs++;
	if (verbose) {
	    fprintf(stderr, "MPIT_Type_memcpy returned error (pack)\n");
	}
    }

    err = MPIT_Type_memcpy(&ts2, 1, mystruct, buffer, MPIT_MEMCPY_TO_USERBUF,
			   0, bufsize);
    if (err != MPI_SUCCESS) {
	errs++;
	if (verbose) {
	    fprintf(stderr, "MPIT_Type_memcpy returned error (unpack)\n");
	}
    }
...
int array_of_structs_test(void)
{
    ...
    MPIT_Type_init(mystruct);
    err = MPIT_Type_memcpy(ts1, 10, mystruct, buffer, MPIT_MEMCPY_FROM_USERBUF,
			   0, bufsize);
    if (err != MPI_SUCCESS) {
	errs++;
	if (verbose) {
    ...
    err = MPIT_Type_memcpy(ts2, 10, mystruct, buffer, MPIT_MEMCPY_TO_USERBUF,
			   0, bufsize);
    if (err != MPI_SUCCESS) {
	errs++;
	if (verbose) {
	    fprintf(stderr, "MPI_Type_memcpy returned error (unpack)\n");
	}
    }
...
int struct_of_structs_test(void)
{
    ...
    MPI_Type_size(finaltype, &bufsize);
    bufsize *= 5;
    ...
    MPIT_Type_init(finaltype);
    err = MPIT_Type_memcpy(buf, 5, finaltype, packbuf, MPIT_MEMCPY_FROM_USERBUF,
			   0, bufsize);
    if (err != MPI_SUCCESS) {
	errs++;
	if (verbose) {
	    fprintf(stderr, "MPIT_Type_memcpy failed (pack)\n");
	}
	return errs;
    }
    ...
    err = MPIT_Type_memcpy(buf2, 5, finaltype, packbuf, MPIT_MEMCPY_TO_USERBUF,
			   0, bufsize);
    if (err != MPI_SUCCESS) {
	errs++;
	if (verbose) {
	    fprintf(stderr, "MPIT_Type_memcpy failed (unpack)\n");
	}
	return errs;
    }
