...
extern double libpgt_g_hrtimer_startvalue;
...
#define HRT_INIT(print, freq)  do {\
  ...
  libpgt_g_hrtimer_startvalue = MPI_Wtime(); \
  freq = 1e9; \
} while(0)
...
#define HRT_GET_TIME(t1, time) time = ( UINT64_T ) ( (t1-libpgt_g_hrtimer_startvalue) * 1e9 );
