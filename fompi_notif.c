 ...
 *
 */
static inline int _foMPI_NotificationReceived_direct(void *args_ptr, int *flag, MPI_Status *status, int wait) {
	/* get the notification matching arguments */
	foMPI_Notification_request_t *fun_args = (foMPI_Notification_request_t *) args_ptr;
	_foMPI_TRACE_LOG(1, "Waiting for notification from:%d tag:%d \n", fun_args->source_rank, fun_args->tag);
...
//TODO: If software agent is needed, here you have to wait only till you collect all the notifications, maybe with Exponential Backoff.
	...
	uint64_t event_inst_id; /* only least significant 4bytes are source-defined*/
	gni_return_t status_gni = GNI_RC_SUCCESS;

	/* poll local or remote queue depending on the matching parameters */
	int local = 
		fun_args->win->create_flavor != foMPI_WIN_FLAVOR_DYNAMIC &&
		(fun_args->source_rank == foMPI_ANY_SOURCE || 
		foMPI_onnode_rank_global_to_local(fun_args->source_rank, fun_args->win) != -1);
	int remote = 
		!local || 
		fun_args->source_rank == foMPI_ANY_SOURCE;
	...
	while (fun_args->count > fun_args->found) {
#ifdef XPMEM
		/*check XPMEM*/
		//TODO: the implementation of foMPI doesn't support xpmem with Dynamic windows
		if (local) {
		...
		/* return or wait depending on the wait flag */
		if (!wait) break;
	}
#endif
	/* store the number of notifications matched so far */
	MPI_Status_set_elements(status, MPI_INT, fun_args->found);
