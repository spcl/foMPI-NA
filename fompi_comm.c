 ...
 *
 * foMPI Communicator
 * This data structure is utilized only for instantiating a comm_world communicator
...
int _foMPI_Comm_create(MPI_Comm comm, _foMPI_Comm *newcomm) {
	...
	*newcomm = (foMPI_Comm_desc_t *) _foMPI_ALLOC(sizeof(foMPI_Comm_desc_t));
...
#ifdef UGNI
	// CRAY FIX for using multiple process in the same node:
	// int cdm_id = glob_info.num_ugni_comm_instances + glob_info.comm_world->commrank + 1000;
	//TODO: check next step for correctness
	int cdm_id = (glob_info.comm_world->commsize * glob_info.num_ugni_comm_instances) + glob_info.comm_world->commrank + glob_info.comm_world->commsize;
	/*
	...
	int device_id = _foMPI_DEFAULT_DEV_ID;
	...
	(*newcomm)->number_of_cq_entries = _foMPI_NUM_SRC_CQ_ENTRIES; //TODO: evaluate if this variable is useful of MACRO is enough
	//TODO: for not wasting space, is possible to create a cache that allocate descriptors only if needed
	_foMPI_ALIGNED_ALLOC( ((void**) &((*newcomm)->data_desc_array) ), (*newcomm)->number_of_cq_entries * sizeof(gni_post_descriptor_t))
	assert((*newcomm)->data_desc_array != NULL);
	_foMPI_ALIGNED_ALLOC( ((void**) &((*newcomm)->data_desc_ptr) ), (*newcomm)->number_of_cq_entries * sizeof(gni_post_descriptor_t*))
	assert((*newcomm)->data_desc_ptr != NULL);
	int pi;
	void * memptr;
	for(pi=0;pi< (*newcomm)->number_of_cq_entries;pi++){
	...
	status_gni = GNI_CdmCreate(cdm_id, glob_info.curr_job.ptag, glob_info.curr_job.cookie,
			_foMPI_DEFAULT_RDMA_MODES, &((*newcomm)->cdm_handle));
  ...
  _check_gni_status(status_gni, GNI_RC_SUCCESS, (char*) __FILE__, __LINE__);
	...
	status_gni = GNI_CqCreate((*newcomm)->nic_handle, (_foMPI_DEFAULT_RDMA_MODES & GNI_CDM_MODE_DUAL_EVENTS==GNI_CDM_MODE_DUAL_EVENTS ) ?  (*newcomm)->number_of_cq_entries*2 : (*newcomm)->number_of_cq_entries , 0, 	_foMPI_SRC_CQ_MODE, NULL, NULL, &((*newcomm)->cq_handle));
	_check_gni_status(status_gni, GNI_RC_SUCCESS, (char*) __FILE__, __LINE__);
	/*
