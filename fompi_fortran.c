...
#ifdef NOTIFIED_ACCESS_SUPPORT
void F77_FUNC_(fompi_start,FOMPI_START)(int* request, int* ierr);
void F77_FUNC_(fompi_start,FOMPI_START)(int* request, int* ierr){
    
    *ierr = foMPI_Start(&glob_info.Fortran_Requests[*request]);
    
}

void F77_FUNC_(fompi_request_free,FOMPI_REQUEST_FREE)(int* request, int* ierr);
void F77_FUNC_(fompi_request_free,FOMPI_REQUEST_FREE)(int* request, int* ierr){
    
    *ierr = foMPI_Request_free(&glob_info.Fortran_Requests[*request]);
    
    if(glob_info.Fortran_Requests[*request] == foMPI_REQUEST_NULL) {
...
void F77_FUNC_(fompi_notify_init,FOMPI_NOTIFY_INIT)(int *win, int *src_rank, int *tag, int *count, int* request, int* ierr);
void F77_FUNC_(fompi_notify_init,FOMPI_NOTIFY_INIT)(int *win, int *src_rank, int *tag, int *count, int* request, int* ierr){

  *request = get_reqidx();

  *ierr = foMPI_Notify_init(glob_info.Fortran_Windows[*win], *src_rank, *tag, *count, &glob_info.Fortran_Requests[*request]);

}

void F77_FUNC_(fompi_put_notify,FOMPI_PUT_NOTIFY)(void *origin_addr, int *origin_count, int *origin_datatype, int *target_rank, MPI_Aint *target_disp, int *target_count, int *target_datatype, int *win, int *tag, int *ierr);
void F77_FUNC_(fompi_put_notify,FOMPI_PUT_NOTIFY)(void *origin_addr, int *origin_count, int *origin_datatype, int *target_rank, MPI_Aint *target_disp, int *target_count, int *target_datatype, int *win, int *tag, int *ierr){
  ...
  *ierr = foMPI_Put_notify(origin_addr, *origin_count, otype, *target_rank, *target_disp, *target_count, ttype, glob_info.Fortran_Windows[*win], *tag);
